...
### Member Functions Summary
	 ...
	 public void			remove_tag(string tag,string upto="close",int occurence=1)
...
### Function Detail
 ...
 #### public void set_attribute(string tag,string att,string val,int occurence=-1)
       ...
       Parameters:
          ...
          occurence=give occurence number n to set attribute to nth tag default is -1 which means last.
       Exceptions:
            throws exceptions if occurence number given 0
            if the tag does nto exist it automatically creates a tag and sets attribute for it.   
 #### public void remove_tag(string tag,string upto="close",int occurence=1)
	 Description:      
	     adds given attribute and value to specified tag inline
	 Parameters:
	    tag=give tagname
	    upto=if upto is open then only opening tag is removed
	    	 if upto is text then both opening tag and text is removed
		 if upto is close the opening tag,text and closing tag are removed.
		 default is close if not specified
	    occurence=give occurence number n to set attribute to nth tag default is -1 which means last.
	 Exceptions:
             throws exceptions if occurence number given 0 or invalid
             if the tag does nto exist it throws an exception
